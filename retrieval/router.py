...
class QueryRouter:
    def classify(self, request: QueryRequest, history: list[dict]) -> RetrievalMode:
        mode = self._resolve_mode(request, history)
        tracker.set_tag("routed_mode", mode.value)
        return mode

    def _resolve_mode(self, request: QueryRequest, history: list[dict]) -> RetrievalMode:
        ...
        if history:
            logger.info("Router received history size=%d", len(history))
            last_query = history[-1]["query"].lower()
            logger.info("Last query from history: %s", last_query)
